...
class ChurchIndexPage(Page):
    ...
    def get_locality_json(self):
        json_locality_url = '/api-localities/?format=json'
...
class OrgPage(Page):
    ...
    office_web = models.CharField(blank=True, max_length=500, help_text="URL or note (e.g. “See main site”)")
    office_web_2 = models.CharField(blank=True, max_length=500, help_text="URL or note (e.g. “See main site”)")
    last_update = models.DateField(null=True, blank=True)
...
class Contact(WagtailCaptchaEmailForm):
    ...
    def send_mail(self, form):
        ...
        Never raises: any exception is logged and the landing page is still shown.
        """
        ...
        try:
            ...
            if not to_list:
                logger.warning('Contact form: No To address set on Contact page. Set it in Wagtail Admin → Pages → Contact → Email.')
            ...
            if not from_addr:
                logger.warning('Resend: Set RESEND_FROM_EMAIL (or Contact from_address in admin). Falling back to Django email.')
                return super().send_mail(form)
            ...
            try:
                ...
                logger.warning('resend package not installed. Falling back to Django email.')
                return super().send_mail(form)
            ...
            logger.info('Contact form: email sent via Resend to %s', to_list)
        except Exception as e:
            logger.exception('Contact form send_mail failed: %s', e)
            try:
                super().send_mail(form)
            except Exception as fallback_e:
                logger.exception('Contact form Django fallback send_mail also failed: %s', fallback_e)
