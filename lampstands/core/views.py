...
@api_view(['GET'])
def fix_workflowstate(request):
    """
    One-time fix endpoint to add missing workflowstate columns for Wagtail 6.4.
    TEMPORARY: No auth required for quick fix - REMOVE AFTER USE
    """
    from django.db import connection
    
    # List of columns that Wagtail 6.4 expects for WorkflowState
    columns_to_add = [
        {
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if not table_exists:
                return Response({'status': 'error', 'message': 'Table wagtailcore_workflowstate does not exist'}, status=400)
...
@api_view(['GET'])
def fix_taskstate(request):
    """
    One-time fix endpoint to add missing taskstate columns for Wagtail 6.4.
    TEMPORARY: No auth required for quick fix - REMOVE AFTER USE
    """
    from django.db import connection
    
    # List of columns that Wagtail 6.4 expects for TaskState
    columns_to_add = [
        {
            'name': 'revision_id',
            'type': 'INTEGER',
            'default': '',
            'null': 'NULL',
            'note': 'ForeignKey to wagtailcore_revision - nullable'
        },
    ]
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute("""
                SELECT EXISTS (
                    SELECT FROM information_schema.tables 
                    WHERE table_schema = 'public' 
                    AND table_name = 'wagtailcore_taskstate'
                );
            """)
            table_exists = cursor.fetchone()[0]
            
            if not table_exists:
                return Response({'status': 'error', 'message': 'Table wagtailcore_taskstate does not exist'}, status=400)
...
@api_view(['GET'])
def fix_revision(request):
    """
    One-time fix endpoint to add missing revision columns for Wagtail 6.4.
    TEMPORARY: No auth required for quick fix - REMOVE AFTER USE
    """
    from django.db import connection
    
    # List of columns that Wagtail 6.4 expects for Revision
    columns_to_add = [
        {
            'name': 'object_str',
            'type': 'TEXT',
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if not table_exists:
                return Response({'status': 'error', 'message': 'Table wagtailcore_revision does not exist'}, status=400)
...
@api_view(['GET'])
def fix_document(request):
    """
    One-time fix endpoint to add missing document columns for Wagtail 6.4.
    TEMPORARY: No auth required for quick fix - REMOVE AFTER USE
    """
    from django.db import connection
    
    # List of columns that Wagtail 6.4 expects for Document
    columns_to_add = [
        {
            'name': 'file_size',
            'type': 'INTEGER',
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if not table_exists:
                return Response({'status': 'error', 'message': 'Table wagtaildocs_document does not exist'}, status=400)
